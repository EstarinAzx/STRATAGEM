...
export function AssistantThinkingMessage(t0) {
  ...
  if (!shouldShowFullThinking) {
    ...
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
      t5 = <Text dimColor={true} italic={true}>{"TRACE // COGNITION"} <CtrlOToExpand /></Text>;
      $[0] = t5;
    } else {
      t5 = $[0];
    }
    let t6;
    if ($[1] !== t4) {
      t6 = <Box marginTop={t4} borderStyle="single" borderColor="dim" borderLeft={true} borderTop={false} borderRight={false} borderBottom={false} paddingLeft={1}>{t5}</Box>;
      $[1] = t4;
      $[2] = t6;
    } else {
      t6 = $[2];
